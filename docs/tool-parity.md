...
| **AI Chat (ChatGPT)** | ❌ (no official build - installer info-skips) | `brew install --cask chatgpt` | `winget install --id 9PLM9XGG6VKS --source msstore` | ❌ | ❌ | `chatgpt_desktop` group, host only. Windows installs ChatGPT Work/Codex; existing ChatGPT Classic is left untouched. The Codex CLI is separate (`chatgpt_cli`) |
