...
if (Get-Command direnv -ErrorAction SilentlyContinue) {
    ...
    # direnv's native hook only supports 'pwsh' (PowerShell 7.2+); Windows PowerShell 5.1
    # has no valid hook target at all ('powershell' is rejected by direnv), so the native
    # hook can never work here. Wire the Invoke-DirenvHook helper above into the prompt instead.
    if (-not $global:__DirenvPromptWrapped) {
        $global:__DirenvOriginalPrompt = $function:prompt
        function global:prompt {
            Invoke-DirenvHook
            & $global:__DirenvOriginalPrompt
        }
        $global:__DirenvPromptWrapped = $true
    }
}
